...
def resolveCrossReferences(interpretedSourcecode):
    assert operations.OPERATION_COUNT == 16, "exhaustive handling of operation types in getCrossReferences( )"
    ...
    for index in range(len(interpretedSourcecode)):
        ...
        if operation[0] == operations.IF_OPERATION:
            stack.append(index)

        elif operation[0] == operations.ELSE_OPERATION:
            relatedIfOperationIndex= stack.pop( ) # getting index of the related if operation
            assert interpretedSourcecode[relatedIfOperationIndex][0] == operations.IF_OPERATION, "`else` can only be used to close if/else blocks"

            interpretedSourcecode[relatedIfOperationIndex]= (operations.IF_OPERATION, index + 1)

            stack.append(index)

        elif operation[0] == operations.WHILE_OPERATION:
            stack.append(index)

        elif operation[0] == operations.DO_OPERATION:
            relatedWhileOperationIndex= stack.pop( )

            # referencing related while operation
            interpretedSourcecode[index]= (operations.DO_OPERATION, relatedWhileOperationIndex)

            stack.append(index)

        elif operation[0] == operations.BLOCK_END_OPERATION:
            relatedOperationIndex= stack.pop( )

            #* in case of if/else blocks
            if(interpretedSourcecode[relatedOperationIndex][0] == operations.IF_OPERATION
                or interpretedSourcecode[relatedOperationIndex][0] == operations.ELSE_OPERATION):
                    ...
                    interpretedSourcecode[index]= (operations.BLOCK_END_OPERATION, index + 1)

            #* in case of `do` operation in while blocks
            elif interpretedSourcecode[relatedOperationIndex][0] == operations.DO_OPERATION:
                ...
                interpretedSourcecode[index]= (operations.BLOCK_END_OPERATION, interpretedSourcecode[relatedOperationIndex][1])

                interpretedSourcecode[relatedOperationIndex]= (operations.DO_OPERATION, index + 1)
